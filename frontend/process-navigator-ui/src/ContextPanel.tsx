...
export function ContextPanel({ processId, node, onClose, onAction, onUpdated, canExecute, canEdit }: ContextPanelProps) {
  ...
  return <aside className="context">
    ...
    {!!artifacts.length && <section><h3>Материалы</h3>{artifacts.map(capability => <button className="artifact" key={capability.id} onClick={() => onAction(`Открыть материал: ${capability.label}`)}><BookOpen size={18}/><span><strong>{capability.label}</strong><small>{capability.artifact?.kind} · версия {capability.artifact?.version}{capability.artifact?.reference ? ` · ${capability.artifact.reference}` : ''}</small></span></button>)}</section>}
  </aside>;
}
