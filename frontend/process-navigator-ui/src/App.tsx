...
import { ChevronRight, Home, Maximize2, Minus, Plus, Search, Upload } from 'lucide-react';
import { loadProcess, loadProcessCatalog } from './api';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    loadProcess(processId, controller.signal).then(setProcess)
      .catch(reason => { if (reason.name !== 'AbortError') setError(reason.message); });
    return () => controller.abort();
  }, [processId]);
  ...
  const imported = async (result: ProcessImportResult) => {
    ...
    notify(`Импортирован процесс «${result.process.name}»`);
  };
  ...
  return <div className="app-shell">
    ...
    {importOpen && (
      <ImportDialog onClose={() => setImportOpen(false)} onImported={imported}/>
    )}
  </div>;
}
