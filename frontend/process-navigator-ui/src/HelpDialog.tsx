...
const capabilities = [
  ...
  ['Адаптер 1С:ERP', 'Настраиваемый локальный HTTP-обмен, проверка связи и защищённые реквизиты.', 'Готово'],
  ['Уведомления и упоминания', 'Персональные упоминания, счётчик событий и переход к элементу.', 'Готово'],
  ['Карта предприятия', 'Группа юридических лиц, их направления и сквозные процессы между организациями.', 'Готово']
  ,['Управление оргструктурой', 'Настройка группы компаний, юридических лиц и направлений без ручного изменения файлов.', 'Готово']
  ,['Иерархия подразделений', 'Дерево подчинённости подразделений и руководящих должностей внутри каждого юридического лица.', 'Готово']
  ,['Оргпривязки дорожек', 'Связь ролей BPMN с юридическими лицами и конкретными подразделениями.', 'Готово']
  ,['Организационный профиль', 'Подразделение и должность сотрудника с подсказками при назначении на дорожки.', 'Готово']
] as const;
